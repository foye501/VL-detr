...
# Train a direct DINO->LM fusion model with LM box+count supervision.
#
# This is intentionally separate from run_compare_dino_ablation.sh:
# - compare script keeps the clean count_only baseline
# - this script is the box_count stress-test / grounding variant
...
OUTPUT_DIR="${OUTPUT_DIR:-qwen3_vl_det/checkpoints_dino_direct_boxcount}"
...
DEBUG_FIRST_BATCH_GENERATE="${DEBUG_FIRST_BATCH_GENERATE:-1}"
DEBUG_FIRST_BATCH_MAX_NEW_TOKENS="${DEBUG_FIRST_BATCH_MAX_NEW_TOKENS:-128}"
...
if [[ "${DEBUG_FIRST_BATCH}" == "1" ]]; then
  ...
  if [[ "${DEBUG_FIRST_BATCH_GENERATE}" == "1" ]]; then
    train_cmd+=(--debug-first-batch-generate)
  else
    train_cmd+=(--no-debug-first-batch-generate)
  fi
  train_cmd+=(--debug-first-batch-max-new-tokens "${DEBUG_FIRST_BATCH_MAX_NEW_TOKENS}")
fi
