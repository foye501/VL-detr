...
# Train a Qwen-visual + DINO attention-fused LM path without DETR.
# This differs from run_dino_direct_boxcount.sh:
# - uses fused visual tokens (Qwen visual memory cross-attended with DINO)
# - injects those fused tokens into the LM placeholders
# - no DETR decoder supervision
...
DINO_LM_NUM_TOKENS="${DINO_LM_NUM_TOKENS:-32}"
...
train_cmd=(
  ...
  --inject-fused-visual-tokens-to-lm
